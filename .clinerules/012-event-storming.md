...
付箋の中に、集約名を記載します。
集約付箋を、関連するコマンド（左側）とイベント（右側）の間に配置します。
...
graph TB
    subgraph MainFlow[メインフロー]
        direction LR
        cmd1[コマンド1を実行する] --> evt1[イベント1が発生した]
        cmd1[コマンド1を実行する] -.- agg1[集約1]
    end

    subgraph SubFlow[サブフロー]
        cmd2[コマンド1を実行する] --> evt2[イベント1が発生した]
        cmd2[コマンド1を実行する] -.- agg2[集約1]
    end
    ...
    class evt1,evt2,evt3,evt4 event;
